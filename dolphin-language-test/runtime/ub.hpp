...
struct UBNamespace {
    ...
    var render(const var& templatePath, const var& data = var()) {
        ...
        if (!dolphin_read_file(templatePath.toString(), fileContent)) {
            return var("Template not found: " + templatePath.toString());
        }
        std::string rendered = Template.render(var(fileContent), data).toString();
        return var(DolphinUBEngine::compileHTML(rendered));
    }
