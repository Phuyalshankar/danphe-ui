...
object DolphinVideo {
    ...
    fun openVideoCamera(ctx: Context) {
        try {
            ...
            Log.i(TAG, "✅ Video Camera Intent launched successfully")
        } catch (e: Throwable) {
            Log.e(TAG, "openVideoCamera failed, trying photo camera fallback...", e)
            try {
                ...
                Log.e(TAG, "Camera fallback intent failed", ex)
            }
        }
    }
    ...
    fun playVideo(ctx: Context, uriString: String) {
        try {
            // Bypass Android StrictMode FileUriExposed policy for local file intents
            val policyBuilder = android.os.StrictMode.VmPolicy.Builder()
            android.os.StrictMode.setVmPolicy(policyBuilder.build())

            val uri = Uri.parse(uriString)
            val mimeType = if (uriString.startsWith("content://")) {
                ctx.contentResolver.getType(uri) ?: "video/*"
            } else {
                "video/*"
            }

            val targetUri = if (uriString.startsWith("content://") || uriString.startsWith("http")) {
                uri
            } else {
            ...
            ctx.startActivity(intent)
            Log.d(TAG, "playVideo launched successfully → $targetUri (mime=$mimeType)")
        } catch (e: Throwable) {
            Log.e(TAG, "playVideo failed: ${e.message}", e)
        }
    }

    /** Play video inline using MediaPlayer */
