 ...
 * Framer Motion Inspired Spring Physics.
 */
object AnimationEngine {
    ...
    fun apply(v: View, animStr: String, parseColor: ((String, Int) -> Int)? = null) {
        ...
        try {
            when {
                ...
                // SPRING SCALE (Framer 'spring' feel)
                animStr.contains("framer-spring") -> {
                ...
                // STAGGERED SLIDE UP (Overshoot)
                animStr.contains("framer-slide-up") -> {
                    v.translationY = 150f; v.alpha = 0f
                    v.animate()
                ...
                // BOUNCE IN
                animStr.contains("framer-bounce") -> {
                    v.scaleX = 0.3f; v.scaleY = 0.3f
                    v.animate()
                        .scaleX(1f).scaleY(1f)
                        .setDuration(800)
                ...
                // Infinite pulse / color breathing
                animStr.contains("infinite") || animStr.contains("pulse") -> pulse(v)

                // Slide variants
                animStr.contains("slide-up") -> slideUp(v)
                animStr.contains("slide")    -> slideIn(v)

                // Scale in
                animStr.contains("scale")  -> scaleIn(v)

                // Bounce
                animStr.contains("bounce") -> bounceIn(v)

                // Shake
                animStr.contains("shake")  -> shake(v)

                // Animated background color: bg-blue-100-200-500ms
                animStr.startsWith("bg-") && parseColor != null -> animateBgColor(v, animStr, parseColor)

                // PROPERTY ANIMATION: animate-w-100-200-500ms
                animStr.startsWith("animate-") -> animateProperty(v, animStr)

                // Default: fade in
                else -> fadeIn(v)
            }
        } catch (e: Exception) {
            // Animation failed — core rendering unaffected
            v.alpha = 1f
        }
    }

    private fun animateProperty(v: View, animStr: String) {
        // Format: animate-w-100-200-500ms, animate-p-10-20-300ms, etc.
    ...
    fun applyBinary(v: View, sig: Int, config: Int) {
        ...
        // Extract 5-bit ID from bits 1, 2, 3, 5, 6 (Mask: 0x6E)
        ...
        try {
            when (animId) {
                ...
                0x20 -> slideIn(v, duration) // Slide Left (Default)
    ...
    // ── Individual Animations (Extended to 20) ──────────────────────────────
    ...
    private fun pulse(v: View, dur: Long = 1000) {
        val anim = ObjectAnimator.ofFloat(v, "scaleX", 1f, 1.1f, 1f)
        val animY = ObjectAnimator.ofFloat(v, "scaleY", 1f, 1.1f, 1f)
