...
function renderTitanMobileSimulator(options = {}) {
    ...
    <div id="${id}" class="titan-mobile-sim-root relative flex flex-col items-center select-none ${className}" style="perspective: 1200px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
        ...
        <div class="sim-toolbar flex items-center justify-between w-full max-w-[400px] mb-3 px-3.5 py-2 bg-slate-900/90 border border-slate-700/80 rounded-2xl backdrop-blur-xl shadow-xl text-xs text-slate-300 z-50">
        ...
        <div id="${id}-chassis" class="titan-sim-chassis relative transition-transform duration-500 ease-out" style="width: ${width}px; height: ${height}px; transform-style: preserve-3d;">
            ...
            <div id="${id}-screen-root" class="absolute inset-[15px] rounded-[36px] bg-black flex flex-col overflow-hidden text-white font-sans select-none z-10" style="pointer-events: auto !important;">
                ...
                <div class="app-mode-ribbon px-2 pt-6 pb-2 bg-slate-950/95 border-b border-slate-800 flex items-center justify-between gap-1 z-40 text-[10.5px] font-bold">
                ...
                <div class="viewport-wrapper flex-1 relative w-full h-full bg-slate-950 overflow-hidden">
                    ...
                    <!-- A. TIKTOK REELS VIEWPORT (100% Guaranteed 60FPS Video Streaming) -->
                    <div id="${id}-view-tiktok" class="view-panel absolute inset-0 flex flex-col bg-black overflow-hidden z-20">
                        <div class="relative w-full h-full flex items-center justify-center bg-zinc-950">
                            <video id="${id}-tiktok-video" autoplay loop muted playsinline class="w-full h-full object-cover cursor-pointer" onclick="window.titanSimToggleVideoPlay(this)">
                                <source src="/api/video?name=tiktok.mp4" type="video/mp4">
                            </video>
                            ...
                            <div class="absolute bottom-6 left-3 right-14 text-white z-30 pointer-events-none drop-shadow-md">
                                <div class="text-xs font-black tracking-wide flex items-center gap-1.5">
                                    <span>@nepal_creator_4k</span>
                                    <span class="px-1 py-0.2 bg-rose-500 text-[8px] rounded font-bold">VERIFIED</span>
                                </div>
                                <p class="text-[10.5px] text-slate-200 mt-1 line-clamp-2">🔥 Trending Reel in Kathmandu! 🇳🇵 Captured for Video Editor.</p>
                            </div>

                            <!-- Action Sidebar -->
                            <div class="absolute right-2 bottom-6 flex flex-col items-center gap-3.5 z-30">
                                <button type="button" onclick="window.titanSimToggleMute('${id}', '${id}-tiktok-video')" class="w-8 h-8 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center text-sm text-white hover:text-cyan-400 transition cursor-pointer" title="Mute/Unmute">
                                    🔊
                                </button>
                            ...
                            <div class="absolute top-3 left-3 right-3 flex items-center justify-between z-40 bg-slate-900/90 backdrop-blur-md px-3 py-1.5 rounded-xl border border-rose-500/60 shadow-xl">
                                <span class="text-[9.5px] font-bold text-rose-300 flex items-center gap-1">🎵 TikTok Video Stream</span>
                                <button type="button" onclick="window.titanSimSendToTimeline('${id}', 'TikTok Reel - Nepal 4K', '/api/video?name=tiktok.mp4')" class="px-2.5 py-1 bg-rose-600 hover:bg-rose-500 text-white rounded-lg text-[9.5px] font-black tracking-wide shadow-lg flex items-center gap-1 transition-all cursor-pointer">
                                    📥 Grab to Timeline (V1)
                                </button>
                            </div>
                        </div>
                    ...
                    <!-- B. FACEBOOK REELS VIEWPORT -->
                    <div id="${id}-view-facebook" class="view-panel absolute inset-0 hidden flex-col bg-slate-950 overflow-hidden z-20">
                        <div class="relative w-full h-full flex items-center justify-center bg-zinc-950">
                            <video id="${id}-fb-video" loop muted playsinline class="w-full h-full object-cover cursor-pointer" onclick="window.titanSimToggleVideoPlay(this)">
                                <source src="/api/video?name=facebook.mp4" type="video/mp4">
                            </video>
                            ...
                            <!-- ⚡ INSTANT TIMELINE GRABBER HUD BUTTON -->
                            <div class="absolute top-3 left-3 right-3 flex items-center justify-between z-40 bg-slate-900/90 backdrop-blur-md px-3 py-1.5 rounded-xl border border-blue-500/60 shadow-xl">
                                <span class="text-[9.5px] font-bold text-blue-300 flex items-center gap-1">📘 FB Reel Stream</span>
                                <button type="button" onclick="window.titanSimSendToTimeline('${id}', 'Facebook Reel Vlog', '/api/video?name=facebook.mp4')" class="px-2.5 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-[9.5px] font-black tracking-wide shadow-lg flex items-center gap-1 transition-all cursor-pointer">
                                    📥 Grab to Timeline (V1)
                                </button>
                            </div>
                        </div>
                    ...
                    <div id="${id}-view-douyin" class="view-panel absolute inset-0 hidden flex-col bg-slate-950 overflow-hidden z-20">
                        <div class="relative w-full h-full flex items-center justify-center bg-zinc-950">
                            <video id="${id}-douyin-video" loop muted playsinline class="w-full h-full object-cover cursor-pointer" onclick="window.titanSimToggleVideoPlay(this)">
                                <source src="/api/video?name=douyin.mp4" type="video/mp4">
                            </video>
                            
                            <div class="absolute bottom-6 left-3 right-14 text-white z-30 pointer-events-none drop-shadow-md">
                                <div class="text-xs font-black tracking-wide text-amber-400 flex items-center gap-1.5">
                                    <span>🇨🇳 抖音 (Douyin) Viral Short</span>
                                </div>
                            ...
                            <!-- ⚡ INSTANT TIMELINE GRABBER HUD BUTTON -->
                            <div class="absolute top-3 left-3 right-3 flex items-center justify-between z-40 bg-slate-900/90 backdrop-blur-md px-3 py-1.5 rounded-xl border border-amber-500/60 shadow-xl">
                                <span class="text-[9.5px] font-bold text-amber-300 flex items-center gap-1">🇨🇳 Douyin Stream</span>
                                <button type="button" onclick="window.titanSimSendToTimeline('${id}', 'Douyin Viral Clip', '/api/video?name=douyin.mp4')" class="px-2.5 py-1 bg-amber-600 hover:bg-amber-500 text-white rounded-lg text-[9.5px] font-black tracking-wide shadow-lg flex items-center gap-1 transition-all cursor-pointer">
                                    📥 Grab to Timeline (V1)
                                </button>
                            </div>
                        </div>
                    ...
                    <!-- D. LIVE BROWSER & SEARCH VIEWPORT -->
                    <div id="${id}-view-browser" class="view-panel absolute inset-0 hidden flex-col bg-slate-950 overflow-hidden z-20">
                        <div class="px-2.5 py-2 bg-slate-900 border-b border-slate-800 flex items-center gap-2">
                            <div class="flex-1 flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-800 border border-slate-700 rounded-xl shadow-inner">
                                ...
                                <button type="button" onclick="window.titanSimRunSearch('${id}', document.getElementById('${id}-browser-search-input').value)" class="text-[10px] font-bold text-cyan-400 hover:text-cyan-300 cursor-pointer">GO</button>
                            </div>
                        </div>

                        <!-- Quick Shortcuts -->
                        <div class="flex items-center gap-1 px-2.5 py-1 bg-slate-950 border-b border-slate-800/80 text-[9.5px] font-mono overflow-x-auto">
                            <button type="button" onclick="window.titanSimNavigate('${id}', 'https://en.wikipedia.org/wiki/Nepal')" class="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 rounded text-slate-300 cursor-pointer">🇳🇵 Nepal</button>
                            <button type="button" onclick="window.titanSimNavigate('${id}', 'https://news.ycombinator.com')" class="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 rounded text-amber-300 cursor-pointer">📰 TechNews</button>
                            <button type="button" onclick="window.titanSimNavigate('${id}', 'https://httpbin.org/html')" class="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 rounded text-cyan-300 cursor-pointer">🌐 HTML5</button>
                        </div>
                ...
                <div class="home-bar-container w-full h-6 flex items-center justify-center z-50 cursor-pointer hover:bg-white/5 transition" onclick="window.titanSimToggleHome('${id}')" title="Home Bar">
