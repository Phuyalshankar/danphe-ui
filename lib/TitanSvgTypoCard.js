...
function renderTitanSvgTypoCard(options = {}) {
    ...
    return `
<svg id="${id}" viewBox="0 0 360 560" width="100%" height="auto" class="select-none overflow-visible block" xmlns="http://www.w3.org/2000/svg">
    <defs>
        <!-- Gradients -->
    ...
    <g id="${id}-header-group" transform="translate(18, 14)">
        ...
        <svg x="2" y="24" width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1" y="1" width="12" height="12" rx="2" fill="#38bdf8" fill-opacity="0.18"/><text x="7" y="10.5" font-size="8.5" font-family="'JetBrains Mono',monospace" font-weight="900" fill="#38bdf8" text-anchor="middle">T</text><line x1="1" y1="6" x2="13" y2="6" stroke="#38bdf8" stroke-width="0.6" stroke-opacity="0.4"/></svg>
        <text x="20" y="34" font-size="10.5" font-family="'JetBrains Mono', monospace" font-weight="900" fill="#ffffff" letter-spacing="0.8">TITAN TYPOGRAPHY STUDIO</text>
    </g>
    ...
    <g id="${id}-input-group" transform="translate(18, 166)">
        <foreignObject x="0" y="0" width="324" height="38">
            <div xmlns="http://www.w3.org/1999/xhtml" class="w-full h-full relative flex items-center">
                ...
                <span class="absolute right-3 text-xs text-amber-400 pointer-events-none">
                     <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.5 1.5L10.5 3.5L4 10H2V8L8.5 1.5Z" stroke="#fbbf24" stroke-width="1.2" stroke-linejoin="round" fill="none"/><path d="M7 3L9 5" stroke="#fbbf24" stroke-width="1" fill="none"/></svg>
                </span>
            </div>
        </foreignObject>
    </g>
        ...
        <foreignObject x="0" y="0" width="324" height="30">
            <div xmlns="http://www.w3.org/1999/xhtml" style="display:flex; gap:2px; width:100%; height:100%; box-sizing:border-box; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
                <button id="${id}-subtab-font" onclick="switchTypoSubTab('font')" 
                        style="flex:1; height:26px; background:${activeTab === 'font' ? '#ea580c' : '#0f172a'}; color:${activeTab === 'font' ? '#ffffff' : '#fb923c'}; border:1px solid ${activeTab === 'font' ? '#fb923c' : '#334155'}; border-radius:5px; font-size:9px; font-weight:900; cursor:pointer;">
                    FONT
                </button>
                <button id="${id}-subtab-stroke" onclick="switchTypoSubTab('stroke')" 
                        style="flex:1.1; height:26px; background:${activeTab === 'stroke' ? '#0284c7' : '#0f172a'}; color:${activeTab === 'stroke' ? '#ffffff' : '#38bdf8'}; border:1px solid ${activeTab === 'stroke' ? '#38bdf8' : '#334155'}; border-radius:5px; font-size:9px; font-weight:900; cursor:pointer;">
                    STROKE
                </button>
                <button id="${id}-subtab-bg" onclick="switchTypoSubTab('bg')" 
                        style="flex:1.1; height:26px; background:${activeTab === 'bg' ? '#7e22ce' : '#0f172a'}; color:${activeTab === 'bg' ? '#ffffff' : '#c084fc'}; border:1px solid ${activeTab === 'bg' ? '#c084fc' : '#334155'}; border-radius:5px; font-size:9px; font-weight:900; cursor:pointer;">
                    BG PAD
                </button>
                <button id="${id}-subtab-grad" onclick="switchTypoSubTab('grad')" 
                        style="flex:1.1; height:26px; background:${activeTab === 'grad' ? '#b45309' : '#0f172a'}; color:${activeTab === 'grad' ? '#ffffff' : '#fbbf24'}; border:1px solid ${activeTab === 'grad' ? '#fbbf24' : '#334155'}; border-radius:5px; font-size:9px; font-weight:900; cursor:pointer;">
                    GRAD
                </button>
                <button id="${id}-subtab-karaoke" onclick="switchTypoSubTab('karaoke')" 
                        style="flex:1.2; height:26px; background:${activeTab === 'karaoke' ? '#047857' : '#0f172a'}; color:${activeTab === 'karaoke' ? '#ffffff' : '#34d399'}; border:1px solid ${activeTab === 'karaoke' ? '#34d399' : '#334155'}; border-radius:5px; font-size:9px; font-weight:900; cursor:pointer;">
                    KARAOKE
                </button>
            </div>
        </foreignObject>
    ...
    <g id="${id}-panel-karaoke" style="${activeTab === 'karaoke' ? '' : 'display:none;'}">
        <foreignObject x="18" y="246" width="324" height="150">
            <div xmlns="http://www.w3.org/1999/xhtml" style="display:flex; flex-direction:column; gap:6px; width:100%; height:100%; box-sizing:border-box; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:4px;">
                    <button onclick="applyViralTypoTemplate('tiktok')" style="height:28px; background:#0f172a; color:#fde047; border:1px solid #eab308; border-radius:5px; font-size:9px; font-weight:900; cursor:pointer;">TikTok Pop</button>
                    <button onclick="applyViralTypoTemplate('netflix')" style="height:28px; background:#0f172a; color:#f87171; border:1px solid #ef4444; border-radius:5px; font-size:9px; font-weight:900; cursor:pointer;">Netflix Cinema</button>
                    <button onclick="applyViralTypoTemplate('news')" style="height:28px; background:#0f172a; color:#38bdf8; border:1px solid #0284c7; border-radius:5px; font-size:9px; font-weight:900; cursor:pointer;">News Ticker</button>
                    <button onclick="applyViralTypoTemplate('gaming')" style="height:28px; background:#0f172a; color:#ec4899; border:1px solid #db2777; border-radius:5px; font-size:9px; font-weight:900; cursor:pointer;">Gaming Neon</button>
                </div>
            </div>
        </foreignObject>
    </g>
    ...
    <g id="${id}-bottom-tabs-dock" transform="translate(20, 410)">
        <foreignObject x="0" y="0" width="320" height="58">
            <div xmlns="http://www.w3.org/1999/xhtml" id="${id}-tabs-viewport" 
                 style="display:flex; align-items:center; gap:3px; width:100%; height:100%; overflow-x:auto; scrollbar-width:none; -ms-overflow-style:none; box-sizing:border-box; padding:2px 2px 4px 2px; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
                
                <button type="button" id="${id}-tab-anim" onclick="switchTypoTab('${id}', 'anim')"
                        style="flex-shrink:0; width:48px; height:50px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px; border-radius:8px; background:#090d16; border:1.2px solid #1e293b; cursor:pointer; overflow:hidden;">
                    <svg width="14" height="14" viewBox="0 0 20 20" fill="none" style="flex-shrink:0;"><polyline points="4,14 8,8 11,12 14,5 16,8" stroke="#38bdf8" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/><circle cx="16" cy="8" r="2" fill="#38bdf8" opacity="0.7"/></svg>
                    <span style="font-size:7.5px; font-weight:900; letter-spacing:0.3px; color:#38bdf8; white-space:nowrap;">ANIM</span>
                </button>

                <button type="button" id="${id}-tab-transform" onclick="switchTypoTab('${id}', 'transform')"
                        style="flex-shrink:0; width:48px; height:50px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px; border-radius:8px; background:#090d16; border:1.2px solid #1e293b; cursor:pointer; overflow:hidden;">
                    <svg width="14" height="14" viewBox="0 0 20 20" fill="none" style="flex-shrink:0;"><rect x="3" y="3" width="14" height="14" rx="2" stroke="#fbbf24" stroke-width="1.4"/><path d="M7 10H13M10 7V13" stroke="#fbbf24" stroke-width="1.4" stroke-linecap="round"/></svg>
                    <span style="font-size:7px; font-weight:900; letter-spacing:0.2px; color:#fbbf24; white-space:nowrap;">XFORM</span>
                </button>

                <button type="button" id="${id}-tab-color" onclick="switchTypoTab('${id}', 'color')"
                        style="flex-shrink:0; width:48px; height:50px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px; border-radius:8px; background:#090d16; border:1.2px solid #1e293b; cursor:pointer; overflow:hidden;">
                    <svg width="14" height="14" viewBox="0 0 20 20" fill="none" style="flex-shrink:0;"><circle cx="7" cy="7" r="4" stroke="#f472b6" stroke-width="1.3" fill="#f472b6" fill-opacity="0.15"/><circle cx="13" cy="8" r="3.5" stroke="#38bdf8" stroke-width="1.3" fill="#38bdf8" fill-opacity="0.15"/><circle cx="10" cy="14" r="3" stroke="#fbbf24" stroke-width="1.3" fill="#fbbf24" fill-opacity="0.15"/></svg>
                    <span style="font-size:7.5px; font-weight:900; letter-spacing:0.3px; color:#f472b6; white-space:nowrap;">COLOR</span>
                </button>

                <button type="button" id="${id}-tab-typo" onclick="switchTypoTab('${id}', 'typo')"
                        style="flex-shrink:0; width:48px; height:50px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px; border-radius:8px; background:#451a03; border:2px solid #f59e0b; box-shadow:0 0 8px rgba(245,158,11,0.5); cursor:pointer; overflow:hidden;">
                    <svg width="14" height="14" viewBox="0 0 20 20" fill="none" style="flex-shrink:0;"><path d="M4 5H16M10 5V16" stroke="#fbbf24" stroke-width="2" stroke-linecap="round"/><path d="M7 16H13" stroke="#fbbf24" stroke-width="1.6" stroke-linecap="round"/></svg>
                    <span style="font-size:7.5px; font-weight:900; letter-spacing:0.3px; color:#fbbf24; white-space:nowrap;">TYPO</span>
                </button>

                <button type="button" id="${id}-tab-vfx" onclick="switchTypoTab('${id}', 'vfx')"
                        style="flex-shrink:0; width:48px; height:50px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px; border-radius:8px; background:#090d16; border:1.2px solid #1e293b; cursor:pointer; overflow:hidden;">
                    <svg width="14" height="14" viewBox="0 0 20 20" fill="none" style="flex-shrink:0;"><path d="M10 2L11.5 8H18L13 11.5L15 18L10 14L5 18L7 11.5L2 8H8.5Z" stroke="#ef4444" stroke-width="1.3" stroke-linejoin="round" fill="#ef4444" fill-opacity="0.12"/></svg>
                    <span style="font-size:7.5px; font-weight:900; letter-spacing:0.3px; color:#ef4444; white-space:nowrap;">VFX</span>
                </button>

                <button type="button" id="${id}-tab-thumb" onclick="switchTypoTab('${id}', 'thumb')"
                        style="flex-shrink:0; width:48px; height:50px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px; border-radius:8px; background:#090d16; border:1.2px solid #1e293b; cursor:pointer; overflow:hidden;">
                    <svg width="14" height="14" viewBox="0 0 20 20" fill="none" style="flex-shrink:0;"><rect x="2" y="4" width="16" height="12" rx="2" stroke="#a855f7" stroke-width="1.4"/><circle cx="7" cy="8.5" r="1.5" fill="#a855f7" opacity="0.8"/><path d="M2 13L6 9L9 12L13 8L18 13" stroke="#a855f7" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    <span style="font-size:7.5px; font-weight:900; letter-spacing:0.3px; color:#a855f7; white-space:nowrap;">THUMB</span>
                </button>
