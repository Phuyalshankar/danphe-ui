 ...
 * 60FPS Screencast + Multi-Phase Touch/Wheel/Key/DOM Scroll + Authenticated Video Blob Fetcher + Audio Guardian
 */
...
class TitanChromiumEngine {
    ...
    initCdpSession() {
        ...
        // 🛡️ Single Active Audio Guardian + Unmute Helper
            ...
            source: `
                ...
                (function() {
                    ...
                    function enforceSingleAudio(media) {
                        ...
                        // Ensure active video is unmuted if playing
                        try {
                            if (activeMedia.paused === false) {
    ...
    performSwipeUp() {
        ...
        // 5. DOM Next & Wheel Trigger (TikTok specific elements)
        this.sendCdp('Runtime.evaluate', {
            expression: `
                (function() {
                    window.scrollBy({ top: 500, behavior: 'smooth' });
                    const nextBtn = document.querySelector('[data-e2e="arrow-right"], button[aria-label*="Next"], button[aria-label*="next"], [data-e2e="feed-arrow-down"], [class*="arrow-down"], [class*="ArrowDown"]');
    ...
    // 🌟 In-Browser Authenticated Video Blob Grabber (TikTok & DRM-Free Streams)
    async fetchActiveVideoBlob() {
        return new Promise((resolve) => {
            const id = ++this.msgId;
            const handler = (raw) => {
                try {
            ...
            this.sendCdp('Runtime.evaluate', {
                expression: `
                    (async function() {
                        ...
                        if (!video) return { success: false, error: 'No video element found on page' };
                        const src = video.currentSrc || video.src || '';
                        const title = document.title || 'TikTok Video';

                        if (!src || src.startsWith('blob:')) {
                            return { success: false, src: src, title: title, isBlob: true };
                        }

                        try {
                            const res = await fetch(src, { credentials: 'include', mode: 'cors' });
                            if (!res.ok) return { success: false, src: src, title: title, status: res.status };
                            const blob = await res.blob();
                            return new Promise((resFn) => {
                                const reader = new FileReader();
                                reader.onloadend = () => {
                                    const base64 = (reader.result || '').split(',')[1];
                                    resFn({ success: true, base64: base64, title: title, src: src });
                                };
                                reader.onerror = () => resFn({ success: false, src: src, title: title, error: 'FileReader failed' });
                                reader.readAsDataURL(blob);
                            });
                        } catch(e) {
                            return { success: false, src: src, title: title, error: e.message };
                        }
                    })()
                `,
                awaitPromise: true,
    ...
    // 🌟 Extract Active Video From Page Context
    ...
    handleCdpMessage(rawMsg) {
        try {
            ...
            if (msg.method === 'Network.responseReceived') {
                ...
                // Strict filter: Exclude all images / avatars / thumbnails
