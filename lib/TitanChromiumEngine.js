 ...
 * 60FPS Screencast + CDP Touch + Persistent Session + Single Active Audio Guardian
 */
...
class TitanChromiumEngine {
    ...
    initCdpSession() {
        ...
        // 🛡️ SINGLE ACTIVE AUDIO GUARDIAN SCRIPT:
        // Automatically prevents background audio overlap!
    ...
    handleCdpMessage(rawMsg) {
        try {
            ...
            if (msg.method === 'Network.responseReceived') {
                ...
                if (mime.startsWith('video/') || url.match(/\.(mp4|m3u8|webm|mov)(\?.*)?$/i) || url.includes('aweme/v1/play') || url.includes('kuaishou.com/video')) {
                    const mediaItem = {
                        url: url,
                        mimeType: mime,
                        size: resp.encodedDataLength || 0,
                        title: path.basename(url.split('?')[0]) || 'Video Stream',
                        platform: url.includes('douyin') ? 'Douyin' : (url.includes('kuaishou') ? 'Kuaishou' : (url.includes('tiktok') ? 'TikTok' : 'Web Video')),
            ...
            switch (event.type) {
                ...
                case 'scroll': {
                    this.sendCdp('Input.synthesizeScrollGesture', {
                        ...
                        yDistance: Math.round(-(event.deltaY || 0)),
                        speed: 1000
                    });
                    break;
                }
