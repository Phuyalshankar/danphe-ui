 ...
 * 60FPS Screencast + Native In-Process WASAPI Audio + Target Tracking + Auto-Dismiss Modal Guardian + Ultra-Smooth Scroll
 */
...
class TitanChromiumEngine {
    ...
    async start() {
        ...
        console.log('[TitanChromiumEngine] Launching browser with Live In-Process Audio: ' + browserPath);
        ...
        console.log('[TitanChromiumEngine] Active with Real Audio & Streaming on ws://localhost:' + this.wsPort);
        return true;
    }
        ...
        return new Promise((resolve, reject) => {
            http.get('http://127.0.0.1:' + this.port + '/json', (res) => {
                ...
                res.on('end', () => {
                    try {
                        const pages = JSON.parse(data);
                        // Pick the active web page target (avoid blank tabs)
                        const page = pages.find(p => p.type === 'page' && p.url && !p.url.startsWith('chrome://') && p.url !== 'about:blank') ||
                        ...
                        console.log('[TitanChromiumEngine] CDP connecting to: ' + page.title + ' (' + page.url + ')');
    ...
    initCdpSession() {
        ...
        this.sendCdp('Network.enable', { maxTotalBufferSize: 100000000, maxResourceBufferSize: 50000000 });
        ...
        // 🛡️ Continuous Anti-Modal Bypass + Audio Unmute Guardian
            ...
            source: `
                ...
                (function() {
                    ...
                    function cleanOverlaysAndUnmute() {
                        // 1. Dismiss TikTok login modal masks
                        const masks = document.querySelectorAll('.DivModalMask, .DivModalContainer, [class*="ModalMask"], [class*="ModalContainer"], [class*="login-container"]');
                        masks.forEach(function(m) { try { m.remove(); } catch(_) {} });

                        const closeBtns = document.querySelectorAll('[data-e2e="modal-close-inner-button"], [aria-label="Close"], button[class*="close"], .login-modal-close');
                        closeBtns.forEach(function(b) { try { b.click(); } catch(_) {} });

                        // 2. Dismiss Facebook cookie/login overlays
                        const fbDialogs = document.querySelectorAll('div[role="dialog"] [aria-label="Close"], [aria-label="Decline optional cookies"], [aria-label="Allow all cookies"], [data-cookiebanner="accept_button"]');
                        fbDialogs.forEach(function(b) { try { b.click(); } catch(_) {} });

                        // Ensure body scrolling is never locked
                        if (document.body && document.body.style.overflow === 'hidden') {
                            document.body.style.overflow = 'auto';
                        }

                        // 3. Auto-Unmute TikTok / Facebook Volume Controls
                        ...
                        if (playing.length > 0) {
                            ...
                            // Pause background videos to prevent overlapping sound
                    ...
                    setInterval(cleanOverlaysAndUnmute, 600);
                })();
            `
        });
    ...
    performSwipeUp() {
        ...
        // 5. DOM Next Trigger & Modal Purge
        this.sendCdp('Runtime.evaluate', {
            expression: `
                (function() {
    ...
    performSwipeDown() {
        ...
        // 5. DOM Prev Trigger & Modal Purge
        this.sendCdp('Runtime.evaluate', {
            expression: `
                (function() {
                    const masks = document.querySelectorAll('.DivModalMask, .DivModalContainer, [class*="ModalMask"], [class*="ModalContainer"]');
