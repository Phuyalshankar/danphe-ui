...
<svg id="${id}" viewBox="0 0 360 560" width="100%" height="auto" xmlns="http://www.w3.org/2000/svg" 
    ...
    <g id="${id}-quick-swatches" transform="translate(18, 350)">
        ...
        <g transform="translate(6, 27)">
            ...
            <rect x="52" y="0" width="22" height="13" rx="3" fill="#047857" stroke="#ffffff" stroke-width="0.7" onclick="selectDirectColor('${id}', '#047857')" style="cursor:pointer;" />
            <rect x="78" y="0" width="22" height="13" rx="3" fill="#0f766e" stroke="#ffffff" stroke-width="0.7" onclick="selectDirectColor('${id}', '#0f766e')" style="cursor:pointer;" />
            <rect x="104" y="0" width="22" height="13" rx="3" fill="#0369a1" stroke="#ffffff" stroke-width="0.7" onclick="selectDirectColor('${id}', '#0369a1')" style="cursor:pointer;" />
            <rect x="130" y="0" width="22" height="13" rx="3" fill="#1e40af" stroke="#ffffff" stroke-width="0.7" onclick="selectDirectColor('${id}', '#1e40af')" style="cursor:pointer;" />
            <rect x="156" y="0" width="22" height="13" rx="3" fill="#5b21b6" stroke="#ffffff" stroke-width="0.7" onclick="selectDirectColor('${id}', '#5b21b6')" style="cursor:pointer;" />
            <rect x="182" y="0" width="22" height="13" rx="3" fill="#86198f" stroke="#ffffff" stroke-width="0.7" onclick="selectDirectColor('${id}', '#86198f')" style="cursor:pointer;" />
            <rect x="208" y="0" width="22" height="13" rx="3" fill="#ffffff" stroke="#cbd5e1" stroke-width="0.7" onclick="selectDirectColor('${id}', '#ffffff')" style="cursor:pointer;" />
            <rect x="234" y="0" width="22" height="13" rx="3" fill="#94a3b8" stroke="#ffffff" stroke-width="0.7" onclick="selectDirectColor('${id}', '#94a3b8')" style="cursor:pointer;" />
            <rect x="260" y="0" width="22" height="13" rx="3" fill="#475569" stroke="#ffffff" stroke-width="0.7" onclick="selectDirectColor('${id}', '#475569')" style="cursor:pointer;" />
            <rect x="286" y="0" width="22" height="13" rx="3" fill="#020617" stroke="#64748b" stroke-width="0.7" onclick="selectDirectColor('${id}', '#020617')" style="cursor:pointer;" />
        </g>
    </g>
    ...
    <g id="${id}-master-btn" transform="translate(18, 400)">
        <g onclick="applyColorToTimeline('${id}')" style="cursor:pointer;" class="active:scale-[0.98] transition-transform">
            <rect x="0" y="0" width="324" height="38" rx="10" fill="none" stroke="#ec4899" stroke-width="1.8" filter="url(#${id}-glow-pink)" />
            <rect x="1" y="1" width="322" height="36" rx="9" fill="url(#${id}-titanium-rim)" stroke="#f472b6" stroke-width="1" />
            <rect x="3" y="3" width="318" height="32" rx="7" fill="#ec4899" fill-opacity="0.25" />
            
            <circle cx="50" cy="19" r="6" fill="#ec4899" filter="url(#${id}-glow-pink)" />
            <circle cx="50" cy="19" r="2.5" fill="#ffffff" />
            <text x="175" y="23" text-anchor="middle" fill="#ffffff" font-size="11.5" font-weight="900" letter-spacing="0.1em">🎨 APPLY COLOR / GRADIENT TO TIMELINE</text>
        </g>
    </g>

    <!-- ═════════════════════════════════════════════════════════════════════════
         LAYER 8: 6 LUXURIOUS 70px WIDE BOTTOM TABS (TRI-CARD DOCK BRIDGING)
    ══════════════════════════════════════════════════════════════════════════ -->
    <g id="${id}-bottom-tabs" transform="translate(18, 448)">
        ...
        <g onclick="scrollColorTabsLeft('${id}')" style="cursor:pointer;" transform="translate(2, 6)">
            ...
            <path d="M 11,26 L 5,31 L 11,36" fill="none" stroke="#ec4899" stroke-width="2" stroke-linecap="round" />
        </g>

        <g onclick="scrollColorTabsRight('${id}')" style="cursor:pointer;" transform="translate(306, 6)">
            <rect x="0" y="0" width="16" height="62" rx="4" fill="#090d16" stroke="#1e293b" stroke-width="1" />
            <path d="M 5,26 L 11,31 L 5,36" fill="none" stroke="#ec4899" stroke-width="2" stroke-linecap="round" />
        </g>
        ...
        <foreignObject x="22" y="4" width="280" height="66">
            <div xmlns="http://www.w3.org/1999/xhtml" 
                ...
                <!-- Tab 1: ⚡ ANIM (Switches to Animation Card) -->
                <button type="button" id="${id}-tab-anim" onclick="switchColorTab('${id}', 'anim')"
                        class="flex-shrink-0 w-[70px] h-[58px] flex flex-col items-center justify-center gap-1 rounded-xl transition-all duration-150 bg-amber-950/80 border border-amber-500/80 hover:border-amber-400 hover:bg-amber-900 shadow">
                    <span class="text-base leading-none">⚡</span>
                    <span class="text-[9px] font-black tracking-wider text-amber-300">ANIM</span>
                </button>

                <!-- Tab 2: 📐 TRANSFORM (Switches to Transform Card) -->
                <button type="button" id="${id}-tab-transform" onclick="switchColorTab('${id}', 'transform')"
                        class="flex-shrink-0 w-[70px] h-[58px] flex flex-col items-center justify-center gap-1 rounded-xl transition-all duration-150 bg-cyan-950/80 border border-cyan-500/80 hover:border-cyan-400 hover:bg-cyan-900 shadow">
                    <span class="text-base leading-none">📐</span>
                    <span class="text-[9px] font-black tracking-wider text-cyan-300">TRANSFORM</span>
                </button>

                <!-- Tab 3: 🎨 SOLID -->
                <button type="button" id="${id}-tab-solid" onclick="switchColorTab('${id}', 'solid')"
                        class="flex-shrink-0 w-[70px] h-[58px] flex flex-col items-center justify-center gap-1 rounded-xl transition-all duration-150 ${activeTab === 'solid' ? 'bg-pink-950 border-2 border-pink-400 shadow-[0_0_12px_rgba(236,72,153,0.6)]' : 'bg-slate-900/90 border border-slate-800 hover:border-slate-700 hover:bg-slate-800'}">
                    <span class="text-base leading-none">🎨</span>
                    <span class="text-[9px] font-black tracking-wider ${activeTab === 'solid' ? 'text-pink-300' : 'text-slate-400'}">SOLID</span>
                </button>

                <!-- Tab 4: 🌈 GRADIENT -->
                <button type="button" id="${id}-tab-gradient" onclick="switchColorTab('${id}', 'gradient')"
                        class="flex-shrink-0 w-[70px] h-[58px] flex flex-col items-center justify-center gap-1 rounded-xl transition-all duration-150 ${activeTab === 'gradient' ? 'bg-purple-950 border-2 border-purple-400 shadow-[0_0_12px_rgba(168,85,247,0.6)]' : 'bg-slate-900/90 border border-slate-800 hover:border-slate-700 hover:bg-slate-800'}">
                    <span class="text-base leading-none">🌈</span>
                    <span class="text-[9px] font-black tracking-wider ${activeTab === 'gradient' ? 'text-purple-300' : 'text-slate-400'}">GRADIENT</span>
                </button>

                <!-- Tab 5: 🌌 MESH -->
                <button type="button" id="${id}-tab-mesh" onclick="switchColorTab('${id}', 'mesh')"
                        class="flex-shrink-0 w-[70px] h-[58px] flex flex-col items-center justify-center gap-1 rounded-xl transition-all duration-150 ${activeTab === 'mesh' ? 'bg-emerald-950 border-2 border-emerald-400 shadow-[0_0_12px_rgba(16,185,129,0.6)]' : 'bg-slate-900/90 border border-slate-800 hover:border-slate-700 hover:bg-slate-800'}">
                    <span class="text-base leading-none">🌌</span>
                    <span class="text-[9px] font-black tracking-wider ${activeTab === 'mesh' ? 'text-emerald-300' : 'text-slate-400'}">MESH</span>
                </button>

                <!-- Tab 6: 💎 WHEEL -->
                <button type="button" id="${id}-tab-wheel" onclick="switchColorTab('${id}', 'wheel')"
                        class="flex-shrink-0 w-[70px] h-[58px] flex flex-col items-center justify-center gap-1 rounded-xl transition-all duration-150 ${activeTab === 'wheel' ? 'bg-indigo-950 border-2 border-indigo-400 shadow-[0_0_12px_rgba(99,102,241,0.6)]' : 'bg-slate-900/90 border border-slate-800 hover:border-slate-700 hover:bg-slate-800'}">
                    <span class="text-base leading-none">💎</span>
                    <span class="text-[9px] font-black tracking-wider ${activeTab === 'wheel' ? 'text-indigo-300' : 'text-slate-400'}">WHEEL</span>
                </button>
