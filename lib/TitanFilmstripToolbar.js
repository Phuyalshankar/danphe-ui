...
const ACTION_TOOLS = [
    ...
    { id: 'duplicate', name: 'Duplicate Clip', shortcut: 'D', group: 'action', desc: 'Duplicate active clip (D)' },
...
function renderStudioVectorIcon(toolId, isCurrent) {
    ...
    switch (toolId) {
        ...
        case 'duplicate':
            return `
            <g transform="translate(10, 3) scale(0.65)">
                <rect x="9" y="9" width="13" height="13" rx="2" ry="2" fill="none" stroke="${stroke}" stroke-width="1.8" />
                <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" fill="none" stroke="${stroke}" stroke-width="1.8" stroke-linecap="round" />
            </g>`;
...
function renderTitanFilmstripToolbar(options = {}) {
            ...
            <g id="${id}-frame-${tool.id}" transform="translate(${bx}, 2.5)" class="cursor-pointer" 
               ...
               onmouseleave="hideFilmstripTooltip()">
                <rect id="${id}-frame-bg-${tool.id}" x="0" y="0" width="${btnW}" height="${btnH}" rx="5" 
                      ...
                      stroke="${isCurrent ? activeBorderColor : '#1e293b'}" stroke-width="${isCurrent ? 1.4 : 0.9}" />
                <text x="${btnW - 3.5}" y="8.5" font-size="6.5" font-weight="900" font-family="monospace" fill="${isCurrent ? activeBorderColor : '#64748b'}" text-anchor="end">${tool.shortcut}</text>
    ...
    const totalDeckWidth = cutGroupWidth + actionGroupWidth + drawGroupWidth + 20;

    return `
<div id="${id}-wrapper" style="position:relative; width:${width}; height:${height}px; background:#040711; border:1px solid #142036; border-radius:7px; overflow:hidden; user-select:none; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; display:flex; align-items:center; justify-content:center;">
    <div id="${id}-track-container" style="width:100%; height:100%; overflow-x:auto; overflow-y:hidden; scrollbar-width:none; display:flex; align-items:center; justify-content:center;">
        <svg id="${id}-svg" viewBox="0 0 ${totalDeckWidth} 32" width="${totalDeckWidth}" height="32" xmlns="http://www.w3.org/2000/svg" style="display:block; margin:0 auto;">
            <!-- Group 1: 4-Way Cutting -->
            <g id="${id}-group-cutting" transform="translate(4, 0.5)">
                <rect x="0" y="0" width="${cutGroupWidth}" height="31" rx="6" fill="#060b18" stroke="#0284c7" stroke-width="1.1" stroke-opacity="0.85" />
                ${cutButtonsSvg}
            </g>
            <!-- Group 2: Actions -->
            <g id="${id}-group-action" transform="translate(${cutGroupWidth + 8}, 0.5)">
                <rect x="0" y="0" width="${actionGroupWidth}" height="31" rx="6" fill="#0c0a09" stroke="#d97706" stroke-width="1.1" stroke-opacity="0.75" />
                ${actionButtonsSvg}
            </g>
            <!-- Group 3: Vector Draw & Shapes -->
            <g id="${id}-group-draw" transform="translate(${cutGroupWidth + actionGroupWidth + 12}, 0.5)">
                <rect x="0" y="0" width="${drawGroupWidth}" height="31" rx="6" fill="#022c22" stroke="#059669" stroke-width="1.1" stroke-opacity="0.85" />
                ${drawButtonsSvg}
            </g>
        </svg>
    </div>
    ...
    <div id="${id}-tooltip" style="display:none; position:fixed; z-index:9999; pointer-events:none; background:#090d16f2; border:1.2px solid #38bdf8; border-radius:6px; padding:4px 8px; box-shadow:0 8px 24px rgba(0,0,0,0.95); backdrop-filter:blur(10px); transform:translate(-50%, -100%); margin-top:-6px;">
        <div style="display:flex; align-items:center; gap:6px;">
            ...
            <span id="${id}-tt-key" style="font-size:8.5px; font-weight:900; background:#0284c7; color:#ffffff; padding:1px 5px; border-radius:3px; font-family:monospace;">[ C ]</span>
        </div>
    </div>
</div>`;
}
