...
class MockCameraModule extends EventEmitter {
    ...
    ingested = [];
    offline = [];
    registerCamera({ cameraId, expectedWidth, expectedHeight }) {
        if (!this.registered.includes(cameraId))
            this.registered.push(cameraId);
    }
    ingestFrame(cameraId, frame) {
...
// Helpers
// ─────────────────────────────────────────────────────────────
function makeMjpegFrame(size = 64) {
...
function makeRtpInterleaved(payload, channel = 0) {
...
function resetMocks() {
    ...
    mockSocket.connect = jest.fn((port, host, cb) => cb());
...
describe('RtspPullerModule', () => {
    ...
    beforeEach(() => {
        ...
        puller = new RtspPullerModule(cam);
    });
    afterEach(() => {
        puller.destroy();
        jest.clearAllTimers();
