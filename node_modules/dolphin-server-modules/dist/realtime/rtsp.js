...
export class RtspPullerModule extends EventEmitter {
    ...
    connectFFmpeg(handle) {
        ...
        try {
            ...
            handle.stats.errorMessage = `ffmpeg not found: ${err.message}`;
            handle.stats.state = 'error';
            this.emit('camera:error', { cameraId, error: handle.stats.errorMessage });
            return;
        }
    ...
    connectTcp(handle) {
        ...
        const sendSetup = () => {
            send(`SETUP ${url}/trackID=1 RTSP/1.0\r\n` +
                `CSeq: ${cseq++}\r\n` +
                ...
                if (rtpBuffer[0] === 0x24 /* '$' */) {
                    ...
                    if (channel === 0) {
                        ...
                        if (rtp.length >= 12) {
                            ...
                            if (mjpegBuffer.length > 2 * 1024 * 1024)
    ...
    // ───────────────────────────────────────────
    forwardFrame(handle, frame) {
        if (handle.stopped || !frame || frame.length === 0)
            return;
...
// Factory
// ─────────────────────────────────────────────────────────────
export function createRtspPuller(cameraModule) {
