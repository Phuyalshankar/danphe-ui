...
function makeFakeWS(overrides = {}) {
    const ws = {
        readyState: 1 /* OPEN */,
        sent: [],
        send(msg) { this.sent.push(msg); },
        close() { if (this.onclose)
            this.onclose(); },
        onopen: null,
        onmessage: null,
        onclose: null,
...
function injectFakeWS(ws) {
    global.WebSocket = class {
        ...
        set onopen(v) { ws.onopen = v; }
        get onopen() { return ws.onopen; }
        set onmessage(v) { ws.onmessage = v; }
        get onmessage() { return ws.onmessage; }
        set onclose(v) { ws.onclose = v; }
        get onclose() { return ws.onclose; }
        set onerror(v) { ws.onerror = v; }
...
describe('AuthHandler', () => {
    ...
    test('login — token र user set गर्छ', async () => {
        global.fetch = makeFetch(200, { accessToken: 'tok1', user: { id: 1, email: 'a@b.com' } });
    ...
    test('register — POST /api/auth/register', async () => {
        global.fetch = makeFetch(200, { success: true });
    ...
    test('me — user update गर्छ', async () => {
        global.fetch = makeFetch(200, { success: true, data: { id: 2 } });
        await c.auth.me();
        expect(c.auth.user).toEqual({ id: 2 });
    });
    test('verify2FA — token set गर्छ', async () => {
        global.fetch = makeFetch(200, { accessToken: 'tok2fa', user: { id: 3 } });
...
describe('Pub/Sub', () => {
    ...
    test('publish — WS मा message पठाउँछ', async () => {
        ...
        const pub = ws.sent.find((s) => {
            try {
                const p = JSON.parse(s);
                return p.topic === 'topic1';
            }
            catch {
                return false;
            }
...
describe('pubFile — chunked upload', () => {
    ...
    test('FILE_UPLOAD_START + CHUNK + DONE messages पठाउँछ', async () => {
        ...
        const start = msgs.find((m) => m.type === 'FILE_UPLOAD_START');
        const chunks = msgs.filter((m) => m.type === 'FILE_UPLOAD_CHUNK');
        const done = msgs.find((m) => m.type === 'FILE_UPLOAD_DONE');
    ...
    test('onProgress callback सही percent call हुन्छ', async () => {
        ...
        const progress = [];
        await c.pubFile('file2', data, 'tiny.bin', (p) => progress.push(p));
...
describe('DolphinStore', () => {
    ...
    test('where filter लाग्छ', () => {
        const col = {
            ...
            items: [], loading: false, success: true, error: null, _filter: null, _sort: null,
            where: (fn) => { col._filter = fn; c.store._applyTransform(col); return col; },
            orderBy: () => col,
            reset: () => col,
        };
        c.store.data.set('filter_test', col);
        col.where((item) => item.active);
    ...
    test('orderBy sort लाग्छ', () => {
        ...
        col.orderBy('id', 'asc');
        expect(col.items.map((i) => i.id)).toEqual([1, 2, 3]);
    });
});
// ─────────────────────────────────────────────────────────────────────────────
// 14. disconnect
