...
function patchDOM(parentElement, newHTML) {
  ...
  const filterNodes = (nList) => Array.from(nList).filter((node) => node.nodeType !== 3 || node.textContent && node.textContent.trim() !== "");
...
// src/dom/template.ts
function preprocessJSX(templateStr) {
  ...
  const convertTag = (match, componentName, attrs) => {
  ...
  result = result.replace(/\bclassName=/g, "class=").replace(/\bonClick=/g, "onclick=").replace(/\bonChange=/g, "onchange=").replace(/\bonInput=/g, "oninput=").replace(/\bonKeyDown=/g, "onkeydown=").replace(/\bonKeyUp=/g, "onkeyup=").replace(/\bonSubmit=/g, "onsubmit=").replace(/\bkey=\{([\s\S]*?)\}/g, 'data-key="{{$1}}"').replace(/\bkey="([\s\S]*?)"/g, 'data-key="$1"');
      ...
      if (template && typeof processedPayload === "object" && processedPayload !== null) {
        ...
        if (arrayToRender) {
          if (el.getAttribute("data-rt-virtual") === "true") {
            if (!el._virtualListenerWired) {
              el._virtualListenerWired = true;
...
// src/dom/module-bindings.ts
function attachModuleBindings(clientProto) {
  clientProto._scanAndLoadModuleBinds = async function() {
    ...
    for (const el of Array.from(elements)) {
      ...
      try {
        const baseUrl = typeof window !== "undefined" ? window.location.href : void 0;
  ...
  clientProto._initSPARouter = function() {
    ...
    const applyPage = async (html, pushUrlOrHash, pushState = true) => {
      ...
      if (allStoreElements.length > 0) {
        ...
        this.uiStores.forEach((store, storeName) => {
          this._updateDOM(`store/${storeName}`, store);
        });
      }
      this._scanStoreBinds();
...
DolphinClient.prototype.hydrateIcons = hydrateIcons;
if (typeof window !== "undefined") {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    setTimeout(() => {
      if (!window.dolphin) {
        ...
        const options = { debug: debugMode, autoConnect: false };
        ...
        const dolphin = new DolphinClient(void 0, void 0, options);
...
export {
  ...
  $$,
  DolphinClient,
  renderTemplate as compileTemplate,
  dolphinElement,
  dolphinQuery,
  hydrateIcons,
  on,
