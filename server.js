...
<body id="master-page-body" class="bg-slate-950 text-slate-100 min-h-screen flex flex-col items-center justify-start p-4 sm:p-8 font-sans transition-all duration-500">
    ...
    <main class="w-full max-w-7xl flex flex-col gap-8">
        ...
        <!-- ═══════════════════════════════════════════════════════════════════════════════
             🌟 SECTION 2.5: 256 HARDWARE ANIMATION OPCODES STUDIO & 1-255 MASTER TESTER
        ═══════════════════════════════════════════════════════════════════════════════ -->
        <section id="section-animations" class="p-6 sm:p-8 bg-gradient-to-r from-slate-900/95 via-cyan-950/40 to-indigo-950/40 rounded-3xl border-2 border-cyan-500/50 flex flex-col gap-6 shadow-[0_0_60px_rgba(34,211,238,0.25)] backdrop-blur-2xl">
            
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-2xl bg-cyan-950 border border-cyan-400/60 flex items-center justify-center text-cyan-400 shadow-[0_0_20px_rgba(34,211,238,0.4)]">
                    ...
                    <div>
                        <h2 class="text-base sm:text-lg font-black uppercase tracking-wider text-white font-mono flex items-center gap-2">
                            <span>⚡</span> 256 Hardware Animation Master Tester (Opcode 0 to 255)
                        </h2>
                        <p class="text-xs text-slate-300 font-mono">1-Byte Binary Protocol &bull; 120 FPS GPU Physics &bull; Enter any Opcode 1 - 255 below</p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <span class="text-xs font-mono bg-cyan-950 text-cyan-300 px-3.5 py-1.5 rounded-xl border border-cyan-700 font-black shadow-inner">0x00 - 0xFF (0-255)</span>
                </div>
            </div>

            <!-- 🎛️ MASTER ANIMATION STAGE (LARGE ICON + OPCODE INPUT 1-255) -->
            <div class="p-6 sm:p-8 bg-slate-950/90 rounded-3xl border-2 border-cyan-500/40 flex flex-col lg:flex-row items-center justify-between gap-8 shadow-2xl">
                
                <!-- Left: HUGE 160px x 160px MASTER ANIMATED ICON BOX -->
                <div class="flex flex-col items-center gap-3">
                    <div class="relative">
                        <!-- Glow Backdrop -->
                        <div class="absolute inset-0 bg-cyan-500/20 blur-2xl rounded-full pointer-events-none"></div>
                        <div id="master-anim-box" class="relative w-40 h-40 sm:w-44 sm:h-44 rounded-3xl bg-gradient-to-br from-slate-900 via-cyan-950 to-slate-950 border-2 border-cyan-400 flex items-center justify-center text-cyan-300 shadow-[0_0_50px_rgba(34,211,238,0.5)] transition-all overflow-visible titan-anim-heartbeat">
                            <div id="master-anim-inner-icon">
                                <svg width="72" height="72" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>
                            </div>
                        </div>
                    </div>
                    <span id="master-anim-name-badge" class="text-xs font-mono font-black text-cyan-300 bg-cyan-950/90 px-3.5 py-1 rounded-full border border-cyan-700 shadow-sm uppercase tracking-wider">HEARTBEAT</span>
                </div>

                <!-- Center: LIVE NUMERIC INPUT (1 TO 255) & RANGE SCRUBBER SLIDER -->
                <div class="flex-1 w-full flex flex-col gap-5 max-w-xl">
                    
                    <!-- Opcode Stepper Controls -->
                    <div class="flex flex-wrap items-center justify-between gap-4 p-4 bg-slate-900/90 rounded-2xl border border-slate-800 shadow-inner">
                        <div class="flex items-center gap-3">
                            <button type="button" onclick="stepOpcode(-1)" class="w-10 h-10 rounded-xl bg-slate-800 hover:bg-cyan-600 hover:text-slate-950 text-white font-mono font-bold text-base transition flex items-center justify-center shadow">◀</button>
                            
                            <!-- 🔢 DIRECT NUMBER INPUT (1 - 255) -->
                            <div class="flex flex-col items-center">
                                <label class="text-[9px] font-mono font-bold uppercase text-cyan-400 mb-1">Opcode (0 - 255)</label>
                                <input type="number" id="opcode-num-input" min="0" max="255" value="1" 
                                       oninput="setOpcodeFromNumber(this.value)" 
                                       class="w-24 text-center font-mono text-2xl font-black bg-slate-950 border-2 border-cyan-400 text-cyan-300 rounded-xl py-1.5 shadow-[0_0_15px_rgba(34,211,238,0.3)] focus:outline-none focus:border-cyan-300">
                            </div>
                        ...
                        <!-- Telemetry Badges -->
                        <div class="flex flex-col gap-1 text-right font-mono">
                            <div class="flex items-center gap-2 justify-end">
                                <span class="text-[10px] text-slate-400">HEX:</span>
                                <span id="telemetry-hex" class="text-sm font-black text-amber-400 bg-amber-950/60 px-2 py-0.5 rounded border border-amber-800">0x01</span>
                            </div>
                            <div class="flex items-center gap-2 justify-end">
                                <span class="text-[10px] text-slate-400">BINARY:</span>
                                <span id="telemetry-bin" class="text-xs font-bold text-emerald-400 bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-800">00000001</span>
                            </div>
                        </div>
                    </div>

                    <!-- 0 to 255 Full Range Scrubber Slider -->
                    <div class="flex flex-col gap-1.5">
                        <div class="flex items-center justify-between text-xs font-mono">
                            <span class="text-slate-400 font-bold">Scrub All 256 Opcodes:</span>
                            <span id="slider-val-label" class="text-cyan-300 font-bold">Opcode #1 / 255</span>
                        </div>
                        <input type="range" id="opcode-slider" min="0" max="255" value="1" 
                               oninput="setOpcodeFromNumber(this.value)" 
                               class="w-full accent-cyan-400 h-2.5 bg-slate-800 rounded-lg cursor-pointer shadow-inner">
                        <div class="flex justify-between text-[9px] font-mono text-slate-600">
                            <span>0x00 (Idle)</span>
                            <span>0x40 (Flow)</span>
                            <span>0x80 (Spring)</span>
                            <span>0xC0 (Arc)</span>
                            <span>0xFF (255)</span>
                        </div>
                    </div>

                    <!-- ⏱️ CLOCK PULSE / DURATION & FREQUENCY CONTROLLER -->
                    <div class="flex flex-col gap-2 p-3.5 bg-slate-900/80 rounded-2xl border border-cyan-500/30 shadow-inner">
                        <div class="flex items-center justify-between text-xs font-mono">
                            <span class="text-cyan-400 font-bold flex items-center gap-1.5">
                                <span>⏱️</span> <span>Clock Pulse / Time Scale:</span>
                            </span>
                            <span id="clock-telemetry-badge" class="text-amber-400 font-bold bg-amber-950/60 px-2 py-0.5 rounded border border-amber-800 text-[11px]">1.0s (1.00 Hz / 60 BPM)</span>
                        </div>
                        
                        <!-- Duration Range Slider (100ms to 4000ms) -->
                        <div class="flex items-center gap-3">
                            <input type="range" id="clock-pulse-slider" min="100" max="4000" step="50" value="1000" 
                                   oninput="setClockPulseMs(this.value)" 
                                   class="w-full accent-amber-400 h-2 bg-slate-800 rounded-lg cursor-pointer">
                            <span id="clock-ms-val" class="text-xs font-mono font-black text-amber-300 min-w-[50px] text-right">1000ms</span>
                        </div>

                        <!-- Quick Tempo & Pulse Presets -->
                        <div class="flex flex-wrap items-center gap-1.5 pt-1">
                            <span class="text-[9px] font-mono text-slate-400 font-bold uppercase">Clock:</span>
                            <button type="button" onclick="setClockPulseMs(250)" class="px-2 py-0.5 bg-slate-800 hover:bg-amber-700 text-[10px] font-mono text-amber-200 rounded border border-slate-700">⚡ 250ms (4x)</button>
                            <button type="button" onclick="setClockPulseMs(500)" class="px-2 py-0.5 bg-slate-800 hover:bg-amber-700 text-[10px] font-mono text-amber-200 rounded border border-slate-700">🚀 500ms (2x)</button>
                            <button type="button" onclick="setClockPulseMs(1000)" class="px-2 py-0.5 bg-slate-800 hover:bg-amber-700 text-[10px] font-mono text-amber-200 rounded border border-slate-700">🎯 1.0s (Normal)</button>
                            <button type="button" onclick="setClockPulseMs(2000)" class="px-2 py-0.5 bg-slate-800 hover:bg-amber-700 text-[10px] font-mono text-amber-200 rounded border border-slate-700">🌊 2.0s (Cinema)</button>
                            <button type="button" onclick="setClockPulseMs(3500)" class="px-2 py-0.5 bg-slate-800 hover:bg-amber-700 text-[10px] font-mono text-amber-200 rounded border border-slate-700">🌌 3.5s (Ambient)</button>
                        </div>
                    </div>

                    <!-- Mode Switch & Quick Icon / Text Chooser -->
                    <div class="flex flex-col gap-2.5 pt-2 border-t border-slate-800/80">
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            <!-- Mode Toggle -->
                            <div class="flex items-center gap-1.5 p-1 bg-slate-900 rounded-xl border border-slate-800">
                                <button type="button" id="btn-mode-icon" onclick="setMasterStageMode('icon')" class="px-3 py-1 bg-cyan-600 text-slate-950 text-xs font-mono font-black rounded-lg shadow transition">🎨 Vector Icon</button>
                                <button type="button" id="btn-mode-text" onclick="setMasterStageMode('text')" class="px-3 py-1 bg-slate-800 text-slate-300 text-xs font-mono font-bold rounded-lg hover:bg-slate-700 transition">✍️ Kinetic Text</button>
                            </div>

                            <!-- Custom Text Input (Shown in Text Mode) -->
                            <div id="custom-text-input-wrap" style="display:none;" class="flex items-center gap-2">
                                <span class="text-[10px] font-mono text-slate-400 font-bold">Text:</span>
                                <input type="text" id="custom-preview-text-input" value="DANPHE TITAN" oninput="updateCustomText(this.value)" 
                                       class="py-1 px-3 bg-slate-950 border border-amber-500/60 text-amber-300 font-mono text-xs font-bold rounded-lg focus:outline-none focus:border-amber-400 w-36 shadow-inner">
                            </div>
                        </div>

                        <!-- Quick Icon Buttons -->
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="text-[10px] font-mono font-bold text-slate-400 uppercase">Quick Pick:</span>
                            <button type="button" onclick="setMasterTestIcon(154)" class="px-2.5 py-1 bg-slate-800 hover:bg-cyan-900 text-xs font-mono rounded-lg text-slate-200 transition">💓 Heart</button>
                            <button type="button" onclick="setMasterTestIcon(153)" class="px-2.5 py-1 bg-slate-800 hover:bg-cyan-900 text-xs font-mono rounded-lg text-slate-200 transition">⚡ Zap</button>
                            <button type="button" onclick="setMasterTestIcon(155)" class="px-2.5 py-1 bg-slate-800 hover:bg-cyan-900 text-xs font-mono rounded-lg text-slate-200 transition">🧠 CPU</button>
                            <button type="button" onclick="setMasterTestIcon(184)" class="px-2.5 py-1 bg-slate-800 hover:bg-cyan-900 text-xs font-mono rounded-lg text-slate-200 transition">🔥 Flame</button>
                            <button type="button" onclick="setMasterTestIcon(10)" class="px-2.5 py-1 bg-slate-800 hover:bg-cyan-900 text-xs font-mono rounded-lg text-slate-200 transition">🛡️ Shield</button>
                            <button type="button" onclick="setMasterTestIcon(150)" class="px-2.5 py-1 bg-slate-800 hover:bg-cyan-900 text-xs font-mono rounded-lg text-slate-200 transition">🌀 Spinner</button>
                        </div>
                    </div>

                </div>

                <!-- Right: JSX EXPORT & AUTO-CYCLE LOOP -->
                <div class="flex flex-col gap-3 w-full lg:w-60">
                    <div class="p-4 bg-slate-900/90 rounded-2xl border border-slate-800 flex flex-col gap-2 shadow-inner">
                        <span class="text-[10px] font-mono uppercase text-slate-400 font-bold">Titan JSX Code</span>
                        <code id="master-anim-code" class="p-2.5 bg-slate-950 border border-slate-800 rounded-xl font-mono text-[11px] text-cyan-300 font-bold select-all">
                            &lt;TitanIcon icon={154} anim={1} /&gt;
                        </code>
                        <span id="master-anim-css-class" class="text-[10px] font-mono text-slate-400">Class: <b class="text-white font-bold">titan-anim-heartbeat</b></span>
                    </div>

                    <!-- Auto-Cycle Play Loop Button -->
                    <button type="button" id="btn-auto-cycle" onclick="toggleAutoCycle()" 
                            class="w-full py-2.5 bg-cyan-600 hover:bg-cyan-500 text-slate-950 font-mono text-xs font-black rounded-xl shadow-[0_0_20px_rgba(34,211,238,0.4)] transition flex items-center justify-center gap-2">
                        <span>▶</span> <span>Auto-Cycle (0-255 Loop)</span>
                    </button>
                </div>

            </div>

            <!-- 20 Core Preset Tiles Grid -->
            <div class="flex flex-col gap-2 pt-2">
                <span class="text-xs font-mono font-bold uppercase text-slate-400">Direct Preset Cards:</span>
        ...
        <!-- ═══════════════════════════════════════════════════════════════════════════════
             🌈 SECTION 2.6: 256 DYNAMIC BACKGROUND & CANVAS GPU ENGINE (OPCODE 0 - 255)
        ═══════════════════════════════════════════════════════════════════════════════ -->
        <section id="section-backgrounds" class="p-6 sm:p-8 bg-gradient-to-r from-slate-900/95 via-purple-950/40 to-slate-900/95 rounded-3xl border-2 border-purple-500/50 flex flex-col gap-6 shadow-[0_0_60px_rgba(168,85,247,0.25)] backdrop-blur-2xl">
            
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-2xl bg-purple-950 border border-purple-400/60 flex items-center justify-center text-purple-400 shadow-[0_0_20px_rgba(168,85,247,0.4)]">
                        <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>
                    </div>
                    <div>
                        <h2 class="text-base sm:text-lg font-black uppercase tracking-wider text-white font-mono flex items-center gap-2">
                            <span>🌈</span> 256 Dynamic Background & Canvas GPU Engine
                        </h2>
                        <p class="text-xs text-slate-300 font-mono">C++ Memory Register 0x4240 &bull; 0-255 Cosine Gradient Formula &bull; 120 FPS Fluid Waves & Mesh</p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <button type="button" id="btn-toggle-page-bg" onclick="togglePageBackgroundMode()" class="px-3.5 py-1.5 bg-purple-600 hover:bg-purple-500 text-slate-950 text-xs font-mono font-black rounded-xl shadow-[0_0_20px_rgba(168,85,247,0.5)] transition flex items-center gap-1.5">
                        <span>🖥️</span> <span>Apply to Full Page BG</span>
                    </button>
                    <span class="text-xs font-mono bg-purple-950 text-purple-300 px-3.5 py-1.5 rounded-xl border border-purple-700 font-black shadow-inner">0x00 - 0xFF</span>
                </div>
            </div>

            <!-- 🎛️ MASTER BACKGROUND VIEWPORT & CONTROLS -->
            <div class="p-6 sm:p-8 bg-slate-950/90 rounded-3xl border-2 border-purple-500/40 flex flex-col lg:flex-row items-center justify-between gap-8 shadow-2xl">
                
                <!-- Left: LIVE 16:9 INTERACTIVE BACKGROUND CANVAS / MESH STAGE -->
                <div class="flex flex-col items-center gap-3 w-full lg:w-96">
                    <div class="relative w-full h-56 rounded-2xl overflow-hidden border-2 border-purple-400 shadow-[0_0_40px_rgba(168,85,247,0.4)]">
                        <!-- Layer 1: GPU Liquid Animated Mesh Layer -->
                        <div id="titan-bg-gpu-layer" class="absolute inset-0 titan-bg-gradient-400 transition-all duration-500"></div>

                        <!-- Layer 2: Real-time 2D Canvas Fluid Waves & Particles -->
                        <canvas id="titan-bg-canvas" width="600" height="360" class="absolute inset-0 w-full h-full object-cover block"></canvas>
                        
                        <!-- Overlay Text / Badge -->
                        <div class="absolute inset-0 bg-black/30 backdrop-blur-[1px] flex flex-col items-center justify-center pointer-events-none p-4 text-center">
                            <span id="bg-live-title" class="font-mono font-black text-lg text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.9)] tracking-widest uppercase">CYBERPUNK NEON</span>
                            <span id="bg-live-formula" class="text-[10px] font-mono text-purple-300 drop-shadow font-bold mt-1">120 FPS GPU Mesh &bull; 0-255 Cosine Engine</span>
                        </div>
                    </div>

                    <!-- 0 to 255 Mathematical Spectrum Continuum Guide -->
                    <div class="flex flex-col gap-1.5 p-3 bg-slate-900/90 rounded-2xl border border-slate-800 w-full text-center font-mono">
                        <div class="flex items-center justify-between text-[10px] font-bold">
                            <span class="text-purple-400">0x00 - 0x2F: 🌸 Lotus Bloom</span>
                            <span class="text-sky-400">0x30 - 0x5F: 🫧 Bubbles</span>
                            <span class="text-emerald-400">0x60 - 0x8F: 🌊 Waves</span>
                        </div>
                        <div class="flex items-center justify-between text-[10px] font-bold">
                            <span class="text-amber-400">0x90 - 0xBF: 🔮 GPU Mesh</span>
                            <span class="text-cyan-400">0xC0 - 0xDF: 🌌 Aurora</span>
                            <span class="text-rose-400">0xE0 - 0xFF: 🪐 Galaxy</span>
                        </div>
                    </div>
                </div>

                <!-- Center: LIVE BG OPCODE INPUT (0 TO 255) & CLOCK SPEED -->
                <div class="flex-1 w-full flex flex-col gap-4 max-w-xl">
                    
                    <!-- Opcode Stepper Controls -->
                    <div class="flex flex-wrap items-center justify-between gap-4 p-4 bg-slate-900/90 rounded-2xl border border-slate-800 shadow-inner">
                        <div class="flex items-center gap-3">
                            <button type="button" onclick="stepBgOpcode(-1)" class="w-10 h-10 rounded-xl bg-slate-800 hover:bg-purple-600 hover:text-slate-950 text-white font-mono font-bold text-base transition flex items-center justify-center shadow">◀</button>
                            
                            <!-- 🔢 DIRECT NUMBER INPUT (0 - 255) -->
                            <div class="flex flex-col items-center">
                                <label class="text-[9px] font-mono font-bold uppercase text-purple-400 mb-1">BG Opcode (0 - 255)</label>
                                <input type="number" id="bg-opcode-input" min="0" max="255" value="0" 
                                       oninput="setBgOpcodeFromNumber(this.value)" 
                                       class="w-24 text-center font-mono text-2xl font-black bg-slate-950 border-2 border-purple-400 text-purple-300 rounded-xl py-1.5 shadow-[0_0_15px_rgba(168,85,247,0.3)] focus:outline-none focus:border-purple-300">
                            </div>
                        ...
                        <div class="flex flex-col gap-1 text-right font-mono">
                            <div class="flex items-center gap-2 justify-end">
                                <span class="text-[10px] text-slate-400">HEX:</span>
                                <span id="bg-telemetry-hex" class="text-sm font-black text-purple-300 bg-purple-950/60 px-2 py-0.5 rounded border border-purple-800">0x00</span>
                            </div>
                            <div class="flex items-center gap-2 justify-end">
                                <span class="text-[10px] text-slate-400">COLOR HARMONY:</span>
                                <span id="bg-telemetry-rgb" class="text-xs font-bold text-amber-400 bg-amber-950/60 px-2 py-0.5 rounded border border-amber-800">Cyber Cyan-Purple</span>
                            </div>
                        </div>
                    </div>

                    <!-- 0 to 255 Full Range BG Scrubber Slider -->
                    <div class="flex flex-col gap-1.5">
                        <div class="flex items-center justify-between text-xs font-mono">
                            <span class="text-slate-400 font-bold">Scrub 256 Procedural Backgrounds:</span>
                            <span id="bg-slider-val-label" class="text-purple-300 font-bold">Preset #0 / 255</span>
                        </div>
                        <input type="range" id="bg-opcode-slider" min="0" max="255" value="0" 
                               oninput="setBgOpcodeFromNumber(this.value)" 
                               class="w-full accent-purple-400 h-2.5 bg-slate-800 rounded-lg cursor-pointer shadow-inner">
                        <div class="flex justify-between text-[9px] font-mono text-slate-600">
                            <span>0x00 (Cyber)</span>
                            <span>0x40 (Aurora)</span>
                            <span>0x80 (Volcano)</span>
                            <span>0xC0 (Synth)</span>
                            <span>0xFF (Gold)</span>
                        </div>
                    </div>

                    <!-- 🎨 CUSTOM BRAND PALETTE INJECTOR (COLOR 1, 2, 3) -->
                    <div class="p-3.5 bg-slate-900/90 rounded-2xl border border-slate-800 flex flex-col gap-2.5 shadow-inner">
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-mono font-bold text-slate-300 flex items-center gap-1.5">
                                <span>🎨</span> <span>Custom Brand Color Injector (Video & Studio Theme):</span>
                            </span>
                            <button type="button" id="btn-toggle-custom-color" onclick="toggleCustomColorMode()" 
                                    class="px-2.5 py-1 bg-slate-800 hover:bg-purple-600 hover:text-slate-950 text-slate-300 text-[10px] font-mono font-black rounded-lg border border-slate-700 transition">
                                <span>⚡</span> <span id="lbl-custom-color-mode">Enable Custom Brand Mode</span>
                            </button>
                        </div>

                        <!-- 3 Color Pickers -->
                        <div class="grid grid-cols-3 gap-2.5">
                            <div class="flex items-center gap-2 p-2 bg-slate-950 rounded-xl border border-slate-800">
                                <input type="color" id="custom-c1-input" value="#06b6d4" oninput="setCustomColor(1, this.value)" class="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0">
                                <div class="flex flex-col">
                                    <span class="text-[9px] font-mono text-slate-400 font-bold">Color 1 (Primary)</span>
                                    <span id="custom-c1-hex" class="text-[11px] font-mono font-black text-cyan-300">#06B6D4</span>
                                </div>
                            </div>
                            <div class="flex items-center gap-2 p-2 bg-slate-950 rounded-xl border border-slate-800">
                                <input type="color" id="custom-c2-input" value="#ec4899" oninput="setCustomColor(2, this.value)" class="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0">
                                <div class="flex flex-col">
                                    <span class="text-[9px] font-mono text-slate-400 font-bold">Color 2 (Secondary)</span>
                                    <span id="custom-c2-hex" class="text-[11px] font-mono font-black text-pink-400">#EC4899</span>
                                </div>
                            </div>
                            <div class="flex items-center gap-2 p-2 bg-slate-950 rounded-xl border border-slate-800">
                                <input type="color" id="custom-c3-input" value="#facc15" oninput="setCustomColor(3, this.value)" class="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0">
                                <div class="flex flex-col">
                                    <span class="text-[9px] font-mono text-slate-400 font-bold">Color 3 (Accent)</span>
                                    <span id="custom-c3-hex" class="text-[11px] font-mono font-black text-amber-300">#FACC15</span>
                                </div>
                            </div>
                        </div>

                        <!-- 6 Quick Brand Presets -->
                        <div class="flex flex-wrap items-center gap-1.5 pt-1 border-t border-slate-800/80">
                            <span class="text-[9px] font-mono text-slate-500 font-bold mr-1">Brand Presets:</span>
                            <button type="button" onclick="applyBrandPreset('#dc2626', '#1d4ed8', '#facc15', '🇳🇵 Nepal Flag Theme')" class="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-rose-300 text-[10px] font-mono font-bold rounded-md transition">🇳🇵 Nepal Crimson</button>
                            <button type="button" onclick="applyBrandPreset('#06b6d4', '#ec4899', '#8b5cf6', '🌌 Cyberpunk Neon')" class="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-cyan-300 text-[10px] font-mono font-bold rounded-md transition">🌌 Cyberpunk</button>
                            <button type="button" onclick="applyBrandPreset('#ca8a04', '#059669', '#fef08a', '👑 Royal Emerald Gold')" class="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-amber-300 text-[10px] font-mono font-bold rounded-md transition">👑 Royal Gold</button>
                            <button type="button" onclick="applyBrandPreset('#f97316', '#db2777', '#4f46e5', '🌅 Sunset Twilight')" class="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-orange-300 text-[10px] font-mono font-bold rounded-md transition">🌅 Sunset</button>
                            <button type="button" onclick="applyBrandPreset('#38bdf8', '#6366f1', '#a5f3fc', '❄️ Arctic Ice')" class="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-sky-300 text-[10px] font-mono font-bold rounded-md transition">❄️ Arctic</button>
                        </div>
                    </div>

                    <!-- ⏱️ BG ANIMATION SPEED & WAVE PULSE -->
                    <div class="flex items-center justify-between gap-4 p-3 bg-slate-900/70 rounded-xl border border-slate-800 text-xs font-mono">
                        <span class="text-slate-400 font-bold">Flow Speed:</span>
                        <div class="flex items-center gap-2">
                            <input type="range" id="bg-speed-slider" min="1" max="50" value="15" 
                                   oninput="setBgSpeed(this.value)" 
                                   class="accent-purple-400 h-2 bg-slate-800 rounded-lg cursor-pointer w-32">
                            <span id="bg-speed-val" class="text-purple-300 font-bold min-w-[36px]">1.5x</span>
                        </div>
                    </div>

                </div>

                <!-- Right: JSX & C++ REGISTER EXPORT -->
                <div class="flex flex-col gap-3 w-full lg:w-60">
                    <div class="p-4 bg-slate-900/90 rounded-2xl border border-slate-800 flex flex-col gap-2 shadow-inner">
                        <span class="text-[10px] font-mono uppercase text-slate-400 font-bold">Titan JSX Code</span>
                        <code id="bg-code-jsx" class="p-2 bg-slate-950 border border-slate-800 rounded-xl font-mono text-[10px] text-purple-300 font-bold select-all overflow-x-auto whitespace-pre">
&lt;TitanBackground 
  opcode={0x00} 
  mode="fluid" 
  speed={1.5} /&gt;
                        </code>
                        <span class="text-[9.5px] font-mono text-slate-500">C++ Reg: <b class="text-slate-300">0x4240 (BG_GRADIENT)</b></span>
                    </div>

                    <button type="button" id="btn-bg-auto-cycle" onclick="toggleBgAutoCycle()" 
                            class="w-full py-2.5 bg-purple-600 hover:bg-purple-500 text-slate-950 font-mono text-xs font-black rounded-xl shadow-[0_0_20px_rgba(168,85,247,0.4)] transition flex items-center justify-center gap-2">
                        <span>▶</span> <span>Auto-Cycle Palettes (0-255)</span>
                    </button>
                </div>

            </div>

            <!-- 12 Top Background Palette Quick Pick Cards -->
            <div class="flex flex-col gap-2 pt-2">
                <span class="text-xs font-mono font-bold uppercase text-slate-400">Curated Palette Themes:</span>
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    <div onclick="selectBgPreset(0, 'CYBERPUNK NEON')" class="p-3 bg-gradient-to-br from-slate-950 via-cyan-950 to-purple-950 rounded-2xl border border-cyan-500/40 hover:border-cyan-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-cyan-300">#0x00</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Cyberpunk Neon</h4>
                    </div>
                    <div onclick="selectBgPreset(1, 'AURORA BOREALIS')" class="p-3 bg-gradient-to-br from-emerald-950 via-teal-950 to-cyan-950 rounded-2xl border border-emerald-500/40 hover:border-emerald-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-emerald-300">#0x01</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Aurora Borealis</h4>
                    </div>
                    <div onclick="selectBgPreset(2, 'VOLCANIC MAGMA')" class="p-3 bg-gradient-to-br from-red-950 via-amber-950 to-orange-950 rounded-2xl border border-red-500/40 hover:border-red-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-rose-300">#0x02</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Volcanic Magma</h4>
                    </div>
                    <div onclick="selectBgPreset(3, 'LIQUID GOLD SHEEN')" class="p-3 bg-gradient-to-br from-amber-950 via-yellow-950 to-slate-950 rounded-2xl border border-amber-500/40 hover:border-amber-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-amber-300">#0x03</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Liquid Gold</h4>
                    </div>
                    <div onclick="selectBgPreset(4, 'MIAMI SYNTHWAVE')" class="p-3 bg-gradient-to-br from-fuchsia-950 via-indigo-950 to-sky-950 rounded-2xl border border-fuchsia-500/40 hover:border-fuchsia-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-fuchsia-300">#0x04</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Miami Synthwave</h4>
                    </div>
                    <div onclick="selectBgPreset(5, 'EMERALD MATRIX')" class="p-3 bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-950 rounded-2xl border border-emerald-500/40 hover:border-emerald-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-emerald-300">#0x05</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Emerald Matrix</h4>
                    </div>
                    <div onclick="selectBgPreset(6, 'OCEANIC TRENCH')" class="p-3 bg-gradient-to-br from-sky-950 via-blue-950 to-slate-950 rounded-2xl border border-sky-500/40 hover:border-sky-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-sky-300">#0x06</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Oceanic Trench</h4>
                    </div>
                    <div onclick="selectBgPreset(7, 'ELECTRIC SUNSET')" class="p-3 bg-gradient-to-br from-rose-950 via-purple-950 to-amber-950 rounded-2xl border border-rose-500/40 hover:border-rose-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-rose-300">#0x07</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Electric Sunset</h4>
                    </div>
                    <div onclick="selectBgPreset(8, 'AMETHYST QUARTZ')" class="p-3 bg-gradient-to-br from-purple-950 via-violet-950 to-slate-950 rounded-2xl border border-purple-500/40 hover:border-purple-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-purple-300">#0x08</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Amethyst Quartz</h4>
                    </div>
                    <div onclick="selectBgPreset(9, 'ACID LIME HIGHWAY')" class="p-3 bg-gradient-to-br from-lime-950 via-emerald-950 to-slate-950 rounded-2xl border border-lime-500/40 hover:border-lime-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-lime-300">#0x09</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Acid Lime Highway</h4>
                    </div>
                    <div onclick="selectBgPreset(10, 'DEEP SPACE NEBULA')" class="p-3 bg-gradient-to-br from-indigo-950 via-slate-950 to-pink-950 rounded-2xl border border-indigo-500/40 hover:border-indigo-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-indigo-300">#0x0A</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Deep Nebula</h4>
                    </div>
                    <div onclick="selectBgPreset(11, 'ARCTIC ICE BLIZZARD')" class="p-3 bg-gradient-to-br from-cyan-950 via-slate-950 to-blue-950 rounded-2xl border border-cyan-500/40 hover:border-cyan-400 transition cursor-pointer text-center">
                        <span class="text-[10px] font-mono font-black text-cyan-300">#0x0B</span>
                        <h4 class="text-xs font-black text-white font-mono mt-1 truncate">Arctic Blizzard</h4>
                    </div>
                </div>
            </div>
